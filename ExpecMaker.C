...
Bool_t ExpecMaker::Process(Long64_t entry)
{
  ...
  if (fname.find("TTJets_SingleLept")!=std::string::npos && genHT>600) return kTRUE; // for not double-counting ttbar 
  if (fname.find("DYJetsToLL_M-50_TuneCUETP8M1")!=std::string::npos && genHT>100) return kTRUE; // for not double-counting DY+jets
