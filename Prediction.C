...
{
  // cout << "resetValues()" << endl;
  resetValues();
  // cout << "fChain->GetTree()->GetEntry(entry)" << endl;
  ...
  if(selectedIDIsoMuonsNum_==1 && selectedIDIsoElectronsNum_==0)
    {
      ...
      muIsoEffVec_ = MuIsoPTActivity_->GetEff( selectedIDIsoMuons->at(0).Pt(),selectedIDIsoMuons_MT2Activity->at(0), useAsymmErrors);
      //  muIsoEffVec_ = MuIsoRelPTDeltaRJet_->GetEff(selectedIDIsoMuonsRelPTJet->at(0), selectedIDIsoMuonsDeltaRJet->at(0), useAsymmErrors);
      muRecoEffVec_ = MuRecoPTActivity_->GetEff(selectedIDIsoMuons->at(0).Pt(),selectedIDIsoMuons_MT2Activity->at(0), useAsymmErrors);
      ...
      elecRecoEffVec_ = ElecRecoPTActivity_->GetEff(selectedIDIsoMuons->at(0).Pt(),selectedIDIsoMuons_MT2Activity->at(0), useAsymmErrors);
      elecIsoEffVec_ = ElecIsoPTActivity_->GetEff(selectedIDIsoMuons->at(0).Pt(),selectedIDIsoMuons_MT2Activity->at(0), useAsymmErrors);
      ...
      /*
    	if(UseTagAndProbeEffIso_)muIsoEff_ = getEff(MuIsoPTActivityTAPMC_, selectedIDIsoMuons->at(0).Pt(),selectedIDIsoMuons_MT2Activity->at(0)); 
    	if(UseTagAndProbeEffReco_)muRecoEff_ = getEff(MuRecoPTActivityTAPMC_, selectedIDIsoMuons->at(0).Pt(),selectedIDIsoMuons_MT2Activity->at(0)); 
    	if(UseTagAndProbeEffReco_)elecRecoEff_ = getEff(ElecRecoPTActivityTAPMC_, selectedIDIsoMuons->at(0).Pt(),selectedIDIsoMuons_MT2Activity->at(0)); 
    	if(UseTagAndProbeEffIso_)elecIsoEff_ = getEff(ElecIsoPTActivityTAPMC_, selectedIDIsoMuons->at(0).Pt(),selectedIDIsoMuons_MT2Activity->at(0)); 
      */
      //for compatibility
  ...
  else if(selectedIDIsoMuonsNum_==0 && selectedIDIsoElectronsNum_==1)
    {
      ...
      elecIsoEffVec_ =  ElecIsoPTActivity_->GetEff(selectedIDIsoElectrons->at(0).Pt(),selectedIDIsoElectrons_MT2Activity->at(0), useAsymmErrors);
      ...
      elecRecoEffVec_ = ElecRecoPTActivity_->GetEff(selectedIDIsoElectrons->at(0).Pt(),selectedIDIsoElectrons_MT2Activity->at(0), useAsymmErrors);
      //elecAccEffVec_ = ElecAccMHTNJets_->GetEff(MHT,NJets, useAsymmErrors);
      if(NJets<6.5) elecAccEffVec_ = ElecAccHTMHT_NJetsLow_->GetEff(HT,MHT, useAsymmErrors);
      else elecAccEffVec_ = ElecAccHTMHT_NJetsHigh_->GetEff(HT,MHT, useAsymmErrors);
      ...
      muRecoEffVec_ = MuRecoPTActivity_->GetEff( selectedIDIsoElectrons->at(0).Pt(), selectedIDIsoElectrons_MT2Activity->at(0), useAsymmErrors);
      muIsoEffVec_ = MuIsoPTActivity_->GetEff( selectedIDIsoElectrons->at(0).Pt(), selectedIDIsoElectrons_MT2Activity->at(0), useAsymmErrors);
      ...
      /*
    	...
    	if(UseTagAndProbeEffReco_) muRecoEff_ = getEff(MuRecoPTActivityTAPMC_, selectedIDIsoMuons->at(0).Pt(), selectedIDIsoElectrons_MT2Activity->at(0)); 
    	if(UseTagAndProbeEffIso_) muIsoEff_ = getEff(MuIsoPTActivityTAPMC_, selectedIDIsoMuons->at(0).Pt(), selectedIDIsoElectrons_MT2Activity->at(0)); 
      */    
      //for compatibility
...
void Prediction::Terminate()
{
  ...
  std::cout << "Saving to " << outPutFile->GetName() << std::endl;
  outPutFile->cd();
  tPrediction_->Write();
...
double Prediction::getEff(TH2D* effTH2D, double xValue, double yValue)
{
  ...
  if(result<0.01)
    {
      std::cout<<"Warning efficiency is: "<<result<<" is smaller than 0.01 for histo: "<<effTH2D->GetName()<<std::endl;
      result =0.01;
    }
  if(result>1)
    {
